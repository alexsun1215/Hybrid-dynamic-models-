...
for i in ['stroke','atrial_fibrillation','chart_order_beta_blockers']:
...
tmp=pd.concat([X_resampled,y_resampled],axis=1)[f_select+['death']]
...
threshold=0.5
clf=lgb.LGBMClassifier(random_state=1,max_depth=1,n_estimators=30)
clf.fit(X_train,y_train)
# y_pred_train1=clf.predict(X_train)
y_pred_train2=clf.predict_proba(X_train)[:,1]
y_pred_train1=[int(i>threshold) for i in y_pred_train2]
...
threshold=0.5
clf=xgb.XGBClassifier(random_state=1,max_depth=1,n_estimators=10,learning_rate=0.05)
clf.fit(X_train,y_train)
# y_pred_train1=clf.predict(X_train)
y_pred_train2=clf.predict_proba(X_train)[:,1]
y_pred_train1=[int(i>threshold) for i in y_pred_train2]
...
threshold=0.5
clf=SVC(random_state=1,probability=True,kernel='linear',C=1e-3,max_iter=1)
clf.fit(X_train,y_train)
# y_pred_train1=clf.predict(X_train)
y_pred_train2=clf.predict_proba(X_train)[:,1]
y_pred_train1=[int(i>threshold) for i in y_pred_train2]
...
import catboost as cgb
clf=cgb.CatBoostClassifier(random_seed=1,depth=3,iterations=50,learning_rate=0.08,verbose=0)
clf.fit(X_train,y_train)
threshold=0.5
# y_pred_train1=clf.predict(X_train)
y_pred_train2=clf.predict_proba(X_train)[:,1]
